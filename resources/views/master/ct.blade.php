                    ...
                    <table class="table datatables-basic w-100">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p">CT (Center Thickness)</th>
...
<div class="modal fade" data-backdrop="static" id="ctModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">CT (Center Thickness)</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="ctForm" method="POST"  enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="multisteps-form__content">
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">    
                                <div class="col-12">
                                    <label for="">CT (Center Thickness) <span class="text-danger">*</span></label>
                                    <input class="form-control"  placeholder="Enter ct" id="ct_name" name="ct_name">
                                    <span class="error badge text-danger" id="ct_nameError"></span>
...
{
   document.getElementById('modalTitle').innerText = 'CT (Center Thickness)';
   $('#ctModal').modal('show');
}

function openEditModal(ct) {
    var ct = JSON.parse(decodeURIComponent(ct));
    
    document.getElementById('ct_name').value = ct.ct_name || '';
    document.getElementById('uid').value = ct.ct_id;
    $('#product_type').val(ct.product_type).trigger('change');
    $('#ctModal').modal('show');
}
...
$("#ctForm").submit(function(e)
{
    ...
    let ct_name = document.getElementById("ct_name" + class_name).value.trim();
    let product_type = document.getElementById("product_type" + class_name).value.trim();


    if (ct_name == '') {
        document.getElementById("ct_nameError" + class_name).textContent = "CT (Center Thickness) name required.";
        document.getElementById("ct_name" + class_name).classList.add("is-invalid");
        isValid = false;
    }
            ...
            },
            {
                ...
                render: function(data, type, full) 
                {
                    return (`<div class="dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">ACTION</button><div class="dropdown-menu">`+
                                ...
                                `<a class="action-delete dropdown-item" href="#"  data-id="` + full['ct_id'] + `">Delete</a>`+
                            `</div></div>`
                    );
                }    
            }
