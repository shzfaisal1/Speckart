                    ...
                    <table class="table datatables-basic w-100">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p">Validity In Days </th>
...
<div class="modal fade" data-backdrop="static" id="validityModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Validity In Days </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="validityForm" method="POST"  enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="multisteps-form__content">
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">    
                                <div class="col-12">
                                    <label for="">Validity In Days <span class="text-danger">*</span></label>
                                    <input class="form-control"  placeholder="Enter validity" id="validity_name" name="validity_name">
                                    <span class="error badge text-danger" id="validity_nameError"></span>
...
{
   document.getElementById('modalTitle').innerText = 'Validity In Days';
   $('#validityModal').modal('show');
}

function openEditModal(validity) {
    var validity = JSON.parse(decodeURIComponent(validity));
    
    document.getElementById('validity_name').value = validity.validity_name || '';
    document.getElementById('uid').value = validity.validity_id;
    $('#product_type').val(validity.product_type).trigger('change');
    $('#validityModal').modal('show');
}


$("#validityForm").submit(function(e)
{
    e.preventDefault(); 
    ...
    let validity_name = document.getElementById("validity_name" + class_name).value.trim();
    let product_type = document.getElementById("product_type" + class_name).value.trim();


    if (validity_name == '') {
        document.getElementById("validity_nameError" + class_name).textContent = "validity name required.";
        document.getElementById("validity_name" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    ...
    let form = $("#validityForm")[0];
    let data = new FormData(form);

    $.ajax({
        type: 'POST',
        url: "{{ route('admin.validity-stored') }}",
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false,
            ...
            },
            {
                ...
                render: function(data, type, full) 
                {
                    return (`<div class="dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">ACTION</button><div class="dropdown-menu">`+
                                ...
                                `<a class="action-delete dropdown-item" href="#"  data-id="` + full['validity_id'] + `">Delete</a>`+
                            `</div></div>`
                    );
                }    
            }
