            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="domestic-orders-header">
                        ...
                        <a href="#" class=" btn" data-toggle="modal" data-target="#designModal">
...
<div class="modal fade" data-backdrop="static" id="designModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Design</h5>
                ...
                <form id="designForm" method="POST"  enctype="multipart/form-data">
                    ...
                    <div class="multisteps-form__content">
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">    
                                <div class="col-12">
                                    <label for="">Design <span class="text-danger">*</span></label>
                                    <input class="form-control"  placeholder="Enter design" id="design_name" name="design_name">
                                    <span class="error badge text-danger" id="design_nameError"></span>
...
{
   document.getElementById('modalTitle').innerText = 'Design';
   $('#designModal').modal('show');
}

function openEditModal(design) {
    var design = JSON.parse(decodeURIComponent(design));
    
    document.getElementById('design_name').value = design.design_name || '';
    document.getElementById('uid').value = design.design_id;
    $('#product_type').val(design.product_type).trigger('change');
    $('#designModal').modal('show');
}
...
$("#designForm").submit(function(e)
{
    ...
    let design_name = document.getElementById("design_name" + class_name).value.trim();
    let product_type = document.getElementById("product_type" + class_name).value.trim();


    if (design_name == '') {
        document.getElementById("design_nameError" + class_name).textContent = "design name required.";
        document.getElementById("design_name" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    ...
    let form = $("#designForm")[0];
    let data = new FormData(form);

    $.ajax({
        type: 'POST',
        url: "{{ route('admin.design-stored') }}",
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false,
            ...
            },
            {
                ...
                render: function(data, type, full) 
                {
                    return (`<div class="dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">ACTION</button><div class="dropdown-menu">`+
                                ...
                                `<a class="action-delete dropdown-item" href="#"  data-id="` + full['design_id'] + `">Delete</a>`+
                            `</div></div>`
                    );
                }    
            }
