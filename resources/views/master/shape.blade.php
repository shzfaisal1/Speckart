            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="domestic-orders-header">
                        ...
                        <a href="#" class=" btn" data-toggle="modal" data-target="#shapeModal">
...
<div class="modal fade" data-backdrop="static" id="shapeModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Shape</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="shapeForm" method="POST"  enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">    
                                <div class="col-12">
                                    ...
                                    <input class="form-control"  placeholder="Enter shape" id="shape_name" name="shape_name">
                                    <span class="error badge text-danger" id="shape_nameError"></span>
                                </div>
                                <div class="col-12 mt-3">
                                    <label for="">Image <span class="text-danger" id="imageStar">*</span></label>
...
{
   document.getElementById('modalTitle').innerText = 'shape';
...
function openEditModal(shape) {
    var shape = JSON.parse(decodeURIComponent(shape));
    
    document.getElementById('shape_name').value = shape.shape_name || '';
    document.getElementById('uid').value = shape.shape_id;
    document.getElementById('imageStar').style.display = 'none';
    document.getElementById('image').value = '';
    // $('#product_type').val(shape.product_type).trigger('change');
    $('#shapeModal').modal('show');
}
...
$("#shapeForm").submit(function(e)
{
    ...
    let shape_name = document.getElementById("shape_name" + class_name).value.trim();
    // let product_type = document.getElementById("product_type" + class_name).value.trim();


    if (shape_name == '') {
        document.getElementById("shape_nameError" + class_name).textContent = "Shape name required.";
        document.getElementById("shape_name" + class_name).classList.add("is-invalid");
        isValid = false;
    }

    let uid = document.getElementById("uid").value;
    let image = document.getElementById("image" + class_name).value;
    if (uid == '' && image == '') {
        document.getElementById("imageError" + class_name).textContent = "Image required.";
        document.getElementById("image" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    ...
    let form = $("#shapeForm")[0];
    let data = new FormData(form);

    $.ajax({
        type: 'POST',
        url: "{{ route('admin.shape-stored') }}",
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false,
            ...
            },
            {
                ...
                render: function(data, type, full) 
                {
                    return (`<div class="dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">ACTION</button><div class="dropdown-menu">`+
                                ...
                                `<a class="action-delete dropdown-item" href="#"  data-id="` + full['shape_id'] + `">Delete</a>`+
                            `</div></div>`
                    );
                }    
            }
