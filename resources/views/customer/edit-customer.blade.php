            ...
            <div class="card-body" style="padding: 5px 10px;">
                ...
                <form id="customerForm" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <hr>
                            <h6 style="color:#6c5ce7;font-weight:700;">🏅 Membership Assignment</h6>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="membership_card_id" class="form-label">Membership Card:</label>
                                <select class="form-control select" name="membership_card_id" id="membership_card_id">
                                    <option value="">-- No Membership --</option>
                                    @foreach($membership_cards as $card)
                                        <option value="{{ $card->card_id }}"
                                            data-bogo="{{ $card->enable_bogo }}"
                                            {{ $customer->membership_card_id == $card->card_id ? 'selected' : '' }}>
                                            {{ $card->card_name }}
                                            (₹{{ $card->price }}/{{ $card->validity_days }} days)
                                            {{ $card->enable_bogo ? '🛍 BOGO' : '' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="membership_expiry" id="membership_expiry"
                                    value="{{ $customer->membership_expiry ? \Carbon\Carbon::parse($customer->membership_expiry)->format('Y-m-d') : '' }}">
                                <small class="text-muted">Leave blank if no membership assigned</small>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="form-label">Current Status:</label>
                                <div id="membership_status_badge" style="margin-top:6px;">
                                    @if($customer->membership_card_id && $customer->membership_expiry && \Carbon\Carbon::parse($customer->membership_expiry)->isFuture())
                                        <span class="badge" style="background:#00b894;color:#fff;padding:6px 14px;border-radius:20px;font-size:13px;">
                                            ✅ Active Membership
                                        </span>
                                        <br><small class="text-muted">Expires: {{ \Carbon\Carbon::parse($customer->membership_expiry)->format('d M Y') }}</small>
                                    @elseif($customer->membership_card_id)
                                        <span class="badge" style="background:#d63031;color:#fff;padding:6px 14px;border-radius:20px;font-size:13px;">
                                            ⚠️ Membership Expired
                                        </span>
                                    @else
                                        <span class="badge" style="background:#b2bec3;color:#fff;padding:6px 14px;border-radius:20px;font-size:13px;">
                                            No Membership
                                        </span>
                                    @endif
                                </div>
