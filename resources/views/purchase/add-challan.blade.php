                    ...
                    <div class="col-md-12">
                        <div class="alert alert-danger ml-0 mr-0">
                            <ul class="mb-0">
                                ...
                                <li>1. Once the challan is added, the inventory will be updated automatically as plus or minus.</li>
                                
                                <li>2. After the challan is created, the record will be displayed in the Pending Challan list.</li>
                                
                                <li>3. When a purchase bill is generated from the pending challan, the barcode will be generated only if the “Generate Barcode” option was set to “Yes” at the time of challan creation.</li>
                ...
                <form id="purchaseForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <input class="form-control"  placeholder="Enter Challan Bill Number" id="challan_no" name="challan_no">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-control select" style="height: 32px !important;" id="tax_rule" name="tax_rule">
                                  ...
                                  <option value="Not Appicable">Not Applicable</option>
                            </select>
                            <span class="error badge text-danger" id="tax_ruleError"></span>
                        </div>
                        <div class="col-md-3">
                            <label for="">Select Recevied Store <span class="text-danger">*</span></label>
                            <select class="form-control select" style="height: 32px !important" id="recevied_store_id" name="recevied_store_id" >
                                <option value="">Select Recevied  Store</option>
                            ...
                            <span class="error badge text-danger" id="recevied_store_idError"></span>
                        </div>
                        
                        <div class="col-md-3">
                            <label for="">Select Billing Store <span class="text-danger">*</span></label>
                            <select class="form-control select" style="height: 32px !important" id="billing_store_id" name="billing_store_id" >
                                <option value="">Select Billing  Store</option>
                              <?php $tbl_store =  DB::table("tbl_store")->where('status',1)->get();  ?>
                               @foreach($tbl_store as $tbl_store)
                                <option value="{{$tbl_store->id}}">{{$tbl_store->store_name}} / ({{$tbl_store->store_id}})</option>
                              @endforeach
                            </select>
                            <span class="error badge text-danger" id="billing_store_idError"></span>
                        </div>
                        
                        <div class="col-md-3">
                            <label for="">Consider As <span class="text-danger">*</span></label>
                            <div class="d-flex">
                                <div class="form-check form-check-inline">
                                  <input class="form-check-input" type="radio" name="consider_as" value="1">
                                  <label class="form-check-label">Interbranch Challan</label>
                                </div>
                                <div class="form-check form-check-inline">
                                  <input class="form-check-input" type="radio" name="consider_as" value="0">
                                  <label class="form-check-label">Stock Transfer</label>
                                </div>
                            </div>
                        </div>    
                    </div>
                                    ...
                                    <tbody>
                                        <tr>
                                           ...
                                           <td>
                                            ...
                                            <input type="hidden" style="width:80px" class="form-control base-price" name="product_base_price[]" placeholder="0.00" readonly>
                                            <input type="hidden" class="form-control gst-amount" name="gst_amt[]" value="0.00" style="border-color:rgba(67, 87, 133, .2);border-radius:8px 0 0 8px;" readonly>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                       <div class="row">
                          ...
                          <div class="col-lg-8">
                            <input type="text" id="modal_bb_price" class="form-control" placeholder="Enter amount">
                          </div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function () {
    ...
    function toggleProductContainer()
    {
        ...
        const billNo       = $('#challan_no').val()?.trim();
        const taxRule      = $('#tax_rule').val()?.trim();
        const billing_store_id  = $('#billing_store_id').val()?.trim();
        const recevied_store_id  = $('#recevied_store_id').val()?.trim();
    
        let enabled = supplierName && billNo && taxRule && billing_store_id && recevied_store_id;
    ...
    $('#supplier_name, #challan_no, #billing_store_id,#recevied_store_id, #tax_rule')
    ...
    {
        let totalQty = 0, totalBase = 0, totalGst = 0, totalPurchase = 0;
        
        $(".base-price, .gst-amounte, .product-qty, .total-purchase-price").each(function() {
        ...
        $('#total_base_amount').val((totalBase*totalQty).toFixed(2));
        $('#total_gst_amount').val((totalGst*totalQty).toFixed(2));
...
$("#purchaseForm").submit(function (e) {
    ...
    let billing_store_id     = document.getElementById("billing_store_id" + class_name).value.trim();
    let recevied_store_id     = document.getElementById("recevied_store_id" + class_name).value.trim();
    ...
    if (challan_no === "") {
        document.getElementById("challan_noError" + class_name).textContent = "Purchase Bill Number required.";
    ...
    if (recevied_store_id === "") {
        document.getElementById("recevied_store_idError" + class_name).textContent = "Select Receive Store.";
        document.getElementById("recevied_store_id" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    
    if (billing_store_id === "") {
        document.getElementById("billing_store_idError" + class_name).textContent = "Select Billing Store.";
        document.getElementById("billing_store_id" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    ...
    $.ajax({
        type: "POST",
        url: "{{ route('admin.add-challan-record') }}",
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false,
        ...
        success: function (response) {
            if ($.isEmptyObject(response.error)) {
                ...
                window.location.href = "{{ route('admin.pending-challan') }}";
            } else {
                document.querySelectorAll(".error").forEach(el => el.textContent = "");
                document.querySelectorAll(".is-invalid").forEach(el => el.classList.remove("is-invalid"));
            }
