...
<body>
    <div class="no-print action-buttons">
        ...
        <a href="{{ route('admin.sale.pdf', ['id' => $saleid, 'idd' => $printtype]) }}" class="btn btn-primary">
                            ...
                            <td style="border: 1px solid black; background-color: #ededed; width:33%; vertical-align: top;" class="print-td-table">
                                <table width="100%" cellspacing="0" cellpadding="0" border="0">
                                    <tbody>
                                        ...
                                        @endif
                                        @if(($sale->bogo_discount ?? 0) > 0)
                                        <tr style="font-size: 12px; color: #00b894;">
                                            <td align="right"><span class="rtl-fix"><span>BOGO Discount</span></span> : <span class="ltr-fix">Rs {{$sale->bogo_discount}}</span></td>
                                        </tr>
                                        @endif
                                        <tr>
                                            <td colspan="2" align="center" style="font-size: 25x; font-weight: bold;"><br><span class="rtl-fix"><span>YOU SAVE</span></span> : <span class="ltr-fix">Rs {{$sale->cart_discount + $sale->loyalty_point_amount + $sale->coupon_amount + $sale->total_discount + ($sale->bogo_discount ?? 0)}}</span></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
                            <td style="vertical-align: top;">
                                <table width="100%" cellspacing="0" cellpadding="0" border="0">
                                    <tbody>
                                        ...
                                        @endif
                                         @if($sale->total_discount > 0 || ($sale->bogo_discount ?? 0) > 0 || $sale->cart_discount > 0 || $sale->coupon_amount > 0 || $sale->loyalty_point_amount > 0)
                                        <tr style="font-size: 17px; font-weight: bold;">
                                            <td align="right"><span><span>Total Discount</span></span> : Rs {{$sale->cart_discount + $sale->loyalty_point_amount + $sale->coupon_amount + $sale->total_discount + ($sale->bogo_discount ?? 0)}}</td>
                                        </tr>
                                         @endif
