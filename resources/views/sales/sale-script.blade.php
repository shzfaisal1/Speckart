...
$(document).ready(function () 
{
    ...
    ------------------------- */
    let debounceTimer;
    let bogoActive = false;      // Global BOGO flag
    let bogoCardName = '';       // Membership card name
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        /* ---- B2B VS B2C CUSTOMER SEPARATION ---- */
                        let custType = data.cust_type || 'B2C';
                        $('#cust_type').val(custType);
                        $('#company_name').val(data.company_name || '');
                        $('#gst_no').val(data.gst_no || '');

                        $('#cust_type_badge').show();
                        if (custType === 'B2B') {
                            $('#cust_type_badge')
                                .removeClass('bg-info bg-secondary')
                                .addClass('bg-primary')
                                .html('🏢 B2B Wholesale Customer');

                            $('.b2b-field').show();
                            $('#aadredeemBtn').hide(); // Loyalty Points not applicable for B2B

                            // B2B Banner
                            bogoActive = false;
                            $('#bogo_banner').show();
                            $('#bogo_active_banner, #bogo_expired_banner, #bogo_none_banner').hide();
                            $('#b2b_company_display').text(data.company_name || data.cust_name);
                            $('#b2b_gst_display').text(data.gst_no || 'N/A');
                            $('#b2b_notice_banner').show();

                        } else {
                            $('#cust_type_badge')
                                .removeClass('bg-primary bg-secondary')
                                .addClass('bg-info')
                                .html('👤 B2C Retail Customer');

                            $('.b2b-field').hide();
                            $('#aadredeemBtn').show(); // Loyalty Points enabled for B2C
                            $('#b2b_notice_banner').hide();

                            /* ---- MEMBERSHIP / BOGO LOGIC FOR B2C ---- */
                            bogoActive    = data.bogo_enabled ? true : false;
                            bogoCardName  = data.membership_card_name || '';

                            $('#bogo_banner').show();
                            $('#bogo_active_banner, #bogo_expired_banner, #bogo_none_banner').hide();

                            if (data.membership_card_id) {
                                if (data.bogo_enabled) {
                                    $('#bogo_card_name').text(bogoCardName);
                                    $('#bogo_active_banner').show();
                                } else {
                                    $('#bogo_expired_banner').show();
                                }
                            } else {
                                $('#bogo_none_banner').show();
                            }
                        }
    ...
    function recalcTotals() {
        ...
        /* ========================
           BOGO LOGIC
           If customer has active BOGO membership and there are 2+ eligible
           items (Frame/Glass/Goggles), the cheapest eligible item becomes FREE.
        ======================== */
        let bogoDiscount = 0;
        const bogoTypes  = ['Frame', 'Glass', 'Goggles'];

        if (bogoActive) {
            let eligibleItems = [];
            $('#saleTable tbody tr').each(function () {
                let ptype = $(this).find('.product-type').val();
                let price = parseNum($(this).find('.sale-price').val());
                if (bogoTypes.includes(ptype) && price > 0) {
                    eligibleItems.push(price);
                }
            });

            if (eligibleItems.length >= 2) {
                // Cheapest eligible item is free
                bogoDiscount = Math.min(...eligibleItems);
                $('#bogo_discount_row').show();
                $('#bogo_discount_display').val(bogoDiscount.toFixed(2));
                $('#bogo_discount').val(bogoDiscount.toFixed(2));
            } else {
                $('#bogo_discount_row').hide();
                $('#bogo_discount_display').val('0.00');
                $('#bogo_discount').val('0');
            }
        } else {
            bogoDiscount = 0;
        ...
        let roundoff      = parseNum($("#roundoff").val());
