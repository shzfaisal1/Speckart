            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="domestic-orders-header">
                        <h3>Subcategories Master</h3>
                        <a href="#" class=" btn" data-toggle="modal" data-target="#subcategoryModal" onclick="openModal('add')">
            ...
            <div class="row">
               <div class="col-lg-12">
                <div class="domestic-orders-table">
                    <table id="subcategories-table" class="table datatables-basic w-100">
                        <thead>
                            <tr>
...
{{-- ══════════════════════════════════════════════
     ADD / EDIT MODAL
     ════════════════════════════════════════════════ --}}
<div class="modal fade" data-backdrop="static" id="subcategoryModal" tabindex="-1" role="dialog" aria-labelledby="subcategoryModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document" style="max-width: 520px;">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title fw-semibold" id="subcategoryModalLabel">
                    <i class="fa fa-plus-circle me-2 text-primary"></i>
                    <span id="modal-title-text">Add Subcategory</span>
                </h5>
            ...
            <form id="subcategoryForm" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="sub_category_id" name="category_id" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="sub_name" name="name"
                               placeholder="e.g. Sunglasses" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" class="form-control" id="sub_image" name="image" accept="image/*">
                        <div class="mt-2" id="current_image_preview"></div>
                    </div>

                    <div class="form-group">
                        <label class="form-label fw-medium">Description</label>
                        <textarea class="form-control" id="sub_description" name="description"
                    ...
                    <div class="form-group">
                        ...
                        <div class="toggle-btn">
                            <input type="checkbox" id="sub_status" name="is_active" class="toggle-switch" value="1" checked>
                            <label for="sub_status">Toggle</label>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="submitBtn">
                        <span id="submitBtnText">Save Subcategory</span>
                    </button>
                </div>
...
$(document).ready(function () {
    ...
    $('#subcategoryForm').on('submit', function (e) {
        ...
        var isEdit = id !== '';
        var url    = isEdit ? `${baseUrl}/${id}` : "{{ route('admin.subcategories.store') }}";
        ...
        $.ajax({
            ...
            complete: function () {
                ...
                $('#submitBtnText').text(isEdit ? 'Update Subcategory' : 'Save Subcategory');
            }
        });
    });
    ...
    $(document).on('click', '.btn-edit-subcategory', function () {
        ...
        $.get(`${baseUrl}/${id}/edit`, function (data) {
            ...
            $('#sub_slug').val(data.slug);
            $('#sub_description').val(data.description ?? '');
            $('#sub_status').prop('checked', data.is_active == 1);
            if(data.image) {
    ...
    $(document).on('click', '.btn-delete-subcategory', function () {
        var id   = $(this).data('id');
        var name = $(this).data('name') ?? 'this subcategory';
    ...
    $(document).on('change', '.toggle-status', function () {
        ...
        Swal.fire({
            title:              newState ? 'Activate this subcategory?' : 'Deactivate this subcategory?',
            text:               newState
    ...
    $('#subcategoryModal').on('hidden.bs.modal', resetForm);
});
...
function openModal(mode) {
    ...
    document.getElementById('submitBtnText').textContent =
        mode === 'add' ? 'Save Subcategory' : 'Update Subcategory';
}
...
function resetForm() {
    ...
    document.getElementById('sub_status').checked = true;
