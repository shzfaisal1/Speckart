        ...
        <div class="card">
            ...
            <div class="row">
               <div class="col-lg-12">
                    ...
                    <div id="lossdataload"></div>
               </div>
            </div>
            <br>
            <div class="row mb-3">
                <div class="col-md-12">
                    <table class="table datatables-basic w-100" style="width:100%">
                    <thead>
                        <tr>
...
function get_lossdata()
{

    $.ajax({
	   type: "POST",
	   url: "{{ route('admin.get-lossdata-report') }}",
	   data: {
	   ...
	   {
		    var main_data=success.lossdata_section;
		    $('#lossdataload').empty();
		    if (success.status === 'success') 
            {
                $('#lossdataload').show();
                $('#lossdataload').append(main_data);
...
let dataListView = $('.datatables-basic')
    ...
    .on('draw.dt', function() 
    {
      $('#processingLoader').hide();
        ...
        "bFilter": false,
        "ajax": {
            url: "{{ route('admin.loss-datatable') }}",
            "dataType": "json",
            "type": "POST",
            "data": function(d) 
            {
...
<script>
        $('#bulkexport').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.loss-excel-download') }}",
                data: {
                    product_type: $('#product_type').val(),
                    store_id: $('#store_id').val(),
                    search: $('#search').val(),
