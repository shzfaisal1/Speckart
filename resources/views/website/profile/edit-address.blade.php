                    ...
                    <form action="{{ route('update_address', $address->id) }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control" id="pincode" name="pincode" value="{{ old('pincode', $address->pincode) }}" required>
                            </div>
                        </div>
                        <label class="form-label">Address Type *</label>
                        <div class="InputGroup"> 
                            <input type="radio" name="type" id="type_home" value="home" {{ old('type', $address->type) == 'home' ? 'checked' : '' }} required />
                            <label for="type_home">Home</label>

                            <input type="radio" name="type" id="type_office" value="office" {{ old('type', $address->type) == 'office' ? 'checked' : '' }} />
                            <label for="type_office">Office</label>
                            
                            <input type="radio" name="type" id="type_other" value="other" {{ old('type', $address->type) == 'other' ? 'checked' : '' }} />
                            <label for="type_other">Other</label>
                        </div>

                        <div class="form-check mt-3 mb-3">
                            <input class="form-check-input" style="width: 20px; height: 20px; margin-right: 10px;" type="checkbox" name="is_default" value="1" id="is_default" {{ (old('is_default') || $address->is_default) ? 'checked' : '' }}>
