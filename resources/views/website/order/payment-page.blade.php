                            ...
                            <details>
                                ...
                                <div class="accordion-content">
                                    <div class="dummy-form">
                                        ...
                                        <div class="row mt-2">
                                            <div class="col">
                                                <input type="text" class="form-control" placeholder="Expiry (MM/YY)"
                                                    name="card_expiry">
                                            </div>
                                            <div class="col">
                            ...
                            <table class="table table-borderless">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>₹{{ number_format(($cartData['frame_subtotal'] ?? 0) + ($cartData['lens_subtotal'] ?? 0), 0) }}</td>
                                    </tr>
                                    <tr>
                                        <td>Total Discount</td>
                                        <td>₹{{ number_format(($cartData['bogo_savings'] ?? 0) + ($cartData['third_item_savings'] ?? 0) + ($cartData['coupon_discount'] ?? 0) + ($cartData['first_frame_free_save'] ?? 0), 0) }}</td>
                                    </tr>
                                    @if(($cartData['loyalty_discount'] ?? 0) > 0)
                                    <tr>
                                        <td>Loyalty Points</td>
                                        <td class="text-success">- ₹{{ number_format($cartData['loyalty_discount'], 0) }}</td>
                                    </tr>
                                    @endif
                                    <tr>
                                        <td><b>Total Payable</b></td>
                                        <td><b>₹{{ number_format($cartData['grand_total'] ?? 0, 0) }}</b></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
    ...
    <script>
      ...
      this.summary.offsetHeight + this.content.offsetHeight
        ...
        $(document).ready(function() {
            // Track selected payment method based on which accordion is open
            var selectedPaymentMethod = 'cod'; // Default to COD
            
            $('details').on('toggle', function() {
                if (this.open) {
                    var summaryText = $(this).find('.accordion-title').first().text().trim().toLowerCase();
                    if (summaryText.indexOf('upi') !== -1) selectedPaymentMethod = 'upi';
                    else if (summaryText.indexOf('card') !== -1) selectedPaymentMethod = 'card';
                    else if (summaryText.indexOf('net banking') !== -1) selectedPaymentMethod = 'netbanking';
                    else if (summaryText.indexOf('cash') !== -1) selectedPaymentMethod = 'cod';
                }
            });
