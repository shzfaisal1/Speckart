<?php
namespace App\Http\Controllers\Admin\product\lens;
...
class LensmasterController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:Product-Contact-Lens', ['only' => ['index']]);
    }
    
    public $view_route = 'products';
    public $view_lens = 'lens';
    
    public function index()
    {
        $store_id = auth()->user()->store_id;
        $setting['page_title'] = 'lens';
    ...
    public function store(Request $request)
    {
        ...
        $folderName = 'lens/product/'.$idgenerate;
        ...
        return response()->json(['success' => 'lens created successfully.']);
    }
    ...
    public function update(Request $request, $uid)
    {
        $validator = Validator::make($request->all(), [
            ...
            'product_name'        => 'required|string|max:255',
            'Purchase_Base_Price' => 'required|string|max:255',
        ...
        $folderName = 'lens/product/' . $request->product_id;
        $folderPath = public_path('uploads/' . $folderName);
        ...
        $product->update($data);
         ...
         return response()->json(['success' => 'lens product updated successfully.']);
    }
    
    public function search(Request $request)
    {
        $store_id = auth()->user()->store_id;
        $totalData = Product::where('status', '1')->where('product_type', 'Lens')->count();
        ...
        {
            $templates = Product::where('status', '1')->where('product_type', 'Lens')->offset($start)
                ->limit($limit)
                ->orderBy('id', 'DESC')
                ->get();
        } else 
        {
            ...
            $templates = Product::where('status', '1')->where('product_type', 'Lens')->where('product_code', 'like', '%' . $search . '%')
            ...
            $totalFiltered = Product::where('status', '1')->where('product_type', 'Lens')->where('product_code', 'like', '%' . $search . '%')
                ->orWhere('product_name', 'like', '%' . $search . '%')
                ->orWhere('Company', 'like', '%' . $search . '%')
                ->orWhere('Quality', 'like', '%' . $search . '%')
                ->orWhere('Purchase_Base_Price', 'like', '%' . $search . '%')
            ...
            foreach ($templates as $template) 
            {
                ...
                if (!empty($template->product_image)) {
                    ...
                    if (is_array($product_images)) {
                        $filePath = asset('uploads/lens/product/' . $template->product_id . '/');
                ...
                $nestedData['Number'] =$template->Number;
                $nestedData['CT'] =$template->CT;
                $nestedData['Type'] =$template->Type;
                $nestedData['Materials'] =$template->Materials;
                $nestedData['Validity'] =$template->Validity;
                $nestedData['Modality'] =$template->Modality;
                $nestedData['WC'] =$template->WC;
                $nestedData['Dk_t'] =$template->Dk_t;
