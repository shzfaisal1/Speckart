<?php
namespace App\Http\Controllers\Admin\product\goggles;
...
class GogglesmasterController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:Product-Goggles', ['only' => ['index']]);
    }
    
    public $view_route = 'products';
    public $view_frame = 'goggles';
    
    public function index()
    {
        $store_id = auth()->user()->store_id;
        $setting['page_title'] = 'Goggles';
        $setting['active'] = 'product code';
        $Product = Product::where('status', '1')->get();
        return view($this->view_route . '/' . $this->view_frame . '/index-1', $setting, compact('Product'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $folderName = 'goggles/product/'.$idgenerate;
        ...
        $product = Product::create([
            ...
            'Temple_Detail' => $request->Temple_Detail,
        ...
        return response()->json(['success' => 'Goggles created successfully.']);
    }
     ...
     public function update(Request $request, $uid)
    {
        ...
        $product = Product::findOrFail($uid);
         ...
         // Folder for uploads
        $folderName = 'goggles/product/' . $product->id; // use product ID for safety
        $folderPath = public_path('uploads/' . $folderName);
        ...
        return response()->json(['success' => 'Goggles product updated successfully.']);
    }
    
    public function search(Request $request)
    {
        $store_id = auth()->user()->store_id;
        $totalData = Product::where('status', '1')->where('product_type', 'Goggles')->count();
        ...
        {
            $templates = Product::where('status', '1')->where('product_type', 'Goggles')->offset($start)
                ->limit($limit)
                ->orderBy('id', 'DESC')
                ->get();
        } else 
        {
            ...
            $templates = Product::where('status', '1')->where('product_type', 'Goggles')->where('product_code', 'like', '%' . $search . '%')
            ...
            $totalFiltered = Product::where('status', '1')->where('product_type', 'Goggles')->where('product_code', 'like', '%' . $search . '%')
                ->orWhere('product_name', 'like', '%' . $search . '%')
                ->orWhere('Company', 'like', '%' . $search . '%')
                ->orWhere('Quality', 'like', '%' . $search . '%')
                ->orWhere('Purchase_Base_Price', 'like', '%' . $search . '%')
            ...
            foreach ($templates as $template) 
            {
                ...
                if (!empty($template->product_image)) {
                    ...
                    if (is_array($product_images)) {
                        $filePath = asset('uploads/goggles/product/' . $template->product_id . '/');
                ...
                $nestedData['Gender'] =$template->Gender;
                $nestedData['Shape'] =$template->Shape;
                $nestedData['Material'] =$template->Material;
                $nestedData['Temple_Detail'] =$template->Temple_Detail;
                $nestedData['Bridge_Size'] =$template->Bridge_Size;
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'status'  => 'success',
            'message' => 'Goggles was successfully deleted',
        ]);
    }
