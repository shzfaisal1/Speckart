<?php
namespace App\Http\Controllers\Admin\product\solution;
...
class SolutionmasterController extends Controller
{
    ...
    {
        $this->middleware('permission:Product-Solution', ['only' => ['index']]);
    }
    
    public $view_route = 'products';
    public $view_solution = 'solution';
    
    public function index()
    {
        $store_id = auth()->user()->store_id;
        $setting['page_title'] = 'solution';
        $setting['active'] = 'product code';
        $Product = Product::where('status', '1')->get();
        return view($this->view_route . '/' . $this->view_solution . '/index-1', $setting, compact('Product'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $folderName = 'solution/product/'.$idgenerate;
        ...
        return response()->json(['success' => 'solution created successfully.']);
    }
     ...
     public function update(Request $request, $uid)
    {
        ...
        $folderName = 'solution/product/' . $request->product_id;
        $folderPath = public_path('uploads/' . $folderName);
        ...
        $product->update($data);
         ...
         return response()->json(['success' => 'solution product updated successfully.']);
    }
    
    public function search(Request $request)
    {
        $store_id = auth()->user()->store_id;
        $totalData = Product::where('status', '1')->where('product_type', 'Solution')->count();
        ...
        {
            $templates = Product::where('status', '1')->where('product_type', 'Solution')->offset($start)
                ->limit($limit)
                ->orderBy('id', 'DESC')
                ->get();
        } else 
        {
            ...
            $templates = Product::where('status', '1')->where('product_type', 'Solution')->where('product_code', 'like', '%' . $search . '%')
            ...
            $totalFiltered = Product::where('status', '1')->where('product_type', 'Solution')->where('product_code', 'like', '%' . $search . '%')
                ->orWhere('product_name', 'like', '%' . $search . '%')
                ->orWhere('Company', 'like', '%' . $search . '%')
                ->orWhere('Quality', 'like', '%' . $search . '%')
                ->orWhere('Purchase_Base_Price', 'like', '%' . $search . '%')
            ...
            foreach ($templates as $template) 
            {
                ...
                if (!empty($template->product_image)) {
                    ...
                    if (is_array($product_images)) {
                        $filePath = asset('uploads/solution/product/' . $template->product_id . '/');
                ...
                $nestedData['Variant'] =$template->Variant;
                $nestedData['Color'] =$template->Color;
                $nestedData['Description'] =$template->Description;
                $nestedData['Packing_Type'] =$template->Packing_Type;
