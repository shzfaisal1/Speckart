<?php
namespace App\Http\Controllers\Admin\product\frame;
...
class FramemasterController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:Product-Frame', ['only' => ['index']]);
    }
    
    public $view_route = 'products';
    public $view_frame = 'frame';
    
    public function index()
    {
        $store_id = auth()->user()->store_id;
        $setting['page_title'] = 'Frame';
        $setting['active'] = 'product code';
        $Product = Product::where('status', '1')->get();
        return view($this->view_route . '/' . $this->view_frame . '/index', $setting);
    }
    ...
    public function store(Request $request)
    {
        ...
        $folderName = 'frame/product/'.$idgenerate;
       ...
       // dd($data1['product_image']);
        ...
        return response()->json(['success' => 'Frame created successfully.']);
    }
    ...
    public function update(Request $request, $uid)
    {
        ...
        $validator = Validator::make($request->all(), [
            'product_code'        => 'required|string|max:225',
            'product_name'        => 'required|string|max:255',
        ]);
        ...
        // Folder for uploads
        $folderName = 'frame/product/' . $product->id; // use product ID for safety
        $folderPath = public_path('uploads/' . $folderName);
        ...
        return response()->json([
            'success' => 'Frame product updated successfully.',
    ...
    {
        $store_id = auth()->user()->store_id;
        $totalData = Product::where('status', '1')->where('product_type', 'Frame')->count();
        ...
        {
            $templates = Product::where('status', '1')->where('product_type', 'Frame')->offset($start)
                ->limit($limit)
                ->orderBy('id', 'DESC')
                ->get();
        } 
        else 
        {
            ...
            $templates = Product::where('status', '1')->where('product_type', 'Frame')->where('product_code', 'like', '%' . $search . '%')
            ...
            $totalFiltered = Product::where('status', '1')->where('product_type', 'Frame')->where('product_code', 'like', '%' . $search . '%')
                ->orWhere('product_name', 'like', '%' . $search . '%')
                ->orWhere('Company', 'like', '%' . $search . '%')
                ->orWhere('Quality', 'like', '%' . $search . '%')
                ->orWhere('Purchase_Base_Price', 'like', '%' . $search . '%')
            ...
            foreach ($templates as $template) 
            {
                ...
                if (!empty($template->product_image)) {
                    ...
                    if (is_array($product_images)) {
                        $filePath = asset('uploads/frame/product/' . $template->product_id . '/');
