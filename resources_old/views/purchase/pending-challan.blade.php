            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="domestic-orders-header">
                        <h3>Pending Challan</h3>
                         @if ($usr->can('Challan-Add'))
                        <a href="{{route('admin.add-challan')}}" class=" btn">
                            ...
                            <div class="row justify-content-between align-items-center mb-3 mt-3 mr-3">
                                ...
                                <div class="col-md-2">
                                    <select class="form-control" style="height: 35px !important;" id="bulk_action" name="bulk_action">
                                        ...
                                        <option value="0">Delete Challan</option>
                                        <option value="1">Update Purchase Price</option>
                                    </select>
                                </div>
                            </div>
                            <tr>
                                <th colspan="9" style="color: #FF0000;" colspan="2">Select checkbox and click on "Delete Challan" button to delete challan <strong>OR</strong> click on "Update Purchase Price" to update price</td>
                            </tr>
                            <tr>
...
let dataListView = $('.datatables-basic')
    ...
    .on('draw.dt', function() 
    {
      $('#processingLoader').hide();
        ...
        "bFilter": false,
        "ajax": {
            url: "{{ route('admin.pending-challan-datatable') }}",
            "dataType": "json",
            "type": "POST",
            "data": function(d) 
            {
    ...
    $('#bulk_action').on('change', function(e) {
        ...
        if (sender_ids.length > 0) 
        {
            if(id == '0')
            {
                ajaxCall("{{ route('admin.challan-destroy') }}",sender_ids);
                $('#bulk_action option:first-child').attr("selected", "selected");
            }
            if(id == '1')
            {
                 ajaxCallss("{{ route('admin.check-same-store') }}",sender_ids);
                $('#bulk_action option:first-child').attr("selected", "selected");
        ...
        if (result.value) {
            $.ajax({
                    ...
                    if (data.status === true) {
                        ...
                        let ids = data.challan_ids;
                
                        let form = document.createElement('form');
                        form.method = 'POST';
                        form.action = "{{ route('admin.update-purchase-of-challan') }}";
