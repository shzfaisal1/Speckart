...
<div class="modal fade" data-backdrop="static" id="coatingModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Coating</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="coatingForm" method="POST"  enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="multisteps-form__content">
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">    
                                <div class="col-12">
                                    <label for="">Coating <span class="text-danger">*</span></label>
                                    <input class="form-control"  placeholder="Enter coating" id="coating_name" name="coating_name">
                                    <span class="error badge text-danger" id="coating_nameError"></span>
...
{
   document.getElementById('modalTitle').innerText = 'Coating';
   $('#coatingModal').modal('show');
}

function openEditModal(coating) {
    var coating = JSON.parse(decodeURIComponent(coating));
    
    document.getElementById('coating_name').value = coating.coating_name || '';
    document.getElementById('uid').value = coating.coating_id;
    $('#product_type').val(coating.product_type).trigger('change');
    $('#coatingModal').modal('show');
}


$("#coatingForm").submit(function(e)
{
    e.preventDefault(); 
    ...
    let coating_name = document.getElementById("coating_name" + class_name).value.trim();
    let product_type = document.getElementById("product_type" + class_name).value.trim();


    if (coating_name == '') {
        document.getElementById("coating_nameError" + class_name).textContent = "Coating name required.";
        document.getElementById("coating_name" + class_name).classList.add("is-invalid");
        isValid = false;
    }
    ...
    let form = $("#coatingForm")[0];
    let data = new FormData(form);

    $.ajax({
        type: 'POST',
        url: "{{ route('admin.coating-stored') }}",
        data: data,
        dataType: "JSON",
        processData: false,
        contentType: false,
            ...
            },
            {
                ...
                render: function(data, type, full) 
                {
                    return (`<div class="dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">ACTION</button><div class="dropdown-menu">`+
                                ...
                                `<a class="action-delete dropdown-item" href="#"  data-id="` + full['coating_id'] + `">Delete</a>`+
                            `</div></div>`
                    );
                }    
            }
