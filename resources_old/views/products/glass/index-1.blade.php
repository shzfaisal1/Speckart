...
@endsection
@section('content')
@php $tbl_setting =  DB::table("tbl_product_code_setting")->where('product_type','Glass')->first();   @endphp
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Add New Glass</h5>
                        ...
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control"  placeholder="Enter Material" id="Material" name="Material" autocomplete="off">
                                ...
                                @endif
                                @if($tbl_setting->Coating == '0')
                                <div class="col-md-4">
                                    <label for="">Coating  </label>
                                    <input class="form-control "  placeholder="Enter Coating" id="Coating" name="Coating">
                                    <div id="coatingListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                    <span class="error badge text-danger" id="CoatingError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->Design == '0')
                                <div class="col-md-4">
                                    <label for="">Design  </label>
                                    <input class="form-control"  placeholder="Enter Design" id="Design" name="Design">
                                    <div id="designListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                    <span class="error badge text-danger" id="DesignError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->Product_Index == '0')
                                <div class="col-md-4">
                                    <label for="">Index  </label>
                                     <input class="form-control"  placeholder="Enter Index" id="Index" name="Index">
                                     <div id="indexListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                     <span class="error badge text-danger" id="IndexError"></span>
                                </div>
                                @endif
...
$(document).ready(function() {
    ...
    setupAutocomplete("Coating", "coatingListName", "{{ route('admin.coatingname-dropdown') }}", "coating_name");
    setupAutocomplete("Design", "designListName", "{{ route('admin.designname-dropdown') }}", "design_name");
    setupAutocomplete("Index", "indexListName", "{{ route('admin.indexname-dropdown') }}", "index_name");
});
</script> 
<script>
function openCreateModal()
{
    document.getElementById('modalTitle').innerText = 'Add New Glass';
    document.getElementById('productForm').action = '{{ route('admin.glassproduct.store') }}';
    document.getElementById('formMethod').value = 'POST';
    document.getElementById('productForm').reset();
    $('#warehouseModal').modal('show');
}

function openEditModal(glass) {
    const glassObj = JSON.parse(decodeURIComponent(glass));

    document.getElementById('modalTitle').innerText = 'Edit Glass Product';
    document.getElementById('productForm').action = `/glassproduct/${glassObj.id}`;
    document.getElementById('formMethod').value = 'PUT';

    
    document.getElementById('Purchase_Base_Price').value = glassObj.Purchase_Base_Price || '';
    document.getElementById('Purchase_Price').value = glassObj.Purchase_Price || '';
    document.getElementById('Retail_Price').value = glassObj.Retail_Price || '';
    document.getElementById('BB_Price').value = glassObj.BB_Price || '';

    if (document.getElementById('product_code')) document.getElementById('product_code').value = glassObj.product_code || '';
    if (document.getElementById('product_name')) document.getElementById('product_name').value = glassObj.product_name || '';
    if (document.getElementById('Company')) document.getElementById('Company').value = glassObj.Company || '';
    if (document.getElementById('Quality')) document.getElementById('Quality').value = glassObj.Quality || '';
    if (document.getElementById('Color')) document.getElementById('Color').value = glassObj.Color || '';
    if (document.getElementById('Material')) document.getElementById('Material').value = glassObj.Material || '';
    if (document.getElementById('Coating')) document.getElementById('Coating').value = glassObj.Coating || '';
    if (document.getElementById('Design')) document.getElementById('Design').value = glassObj.Design || '';
    if (document.getElementById('Index')) document.getElementById('Index').value = glassObj.Index || '';
    if (document.getElementById('Description')) document.getElementById('Description').value = glassObj.Description || '';

    // Convert boolean to string in case values are booleans
    setRadioValue("Track_Inventory", glassObj.Track_Inventory);
    setRadioValue("Allow_Negative_Inventory", glassObj.Allow_Negative_Inventory);
    document.getElementById('uid').value = glassObj.id;
    document.getElementById('product_id').value = glassObj.product_id;
    
    try {
        var images = JSON.parse(glassObj.product_image);
        ...
        if (Array.isArray(images) && images.length > 0) {
            images.forEach(function(imageName) {
                var imagePath = `/public/uploads/glass/product/${glassObj.product_id}/${imageName}`;
    ...
    if (glassObj.main_image) {
        var mainImagePath = `/public/uploads/glass/product/${glassObj.product_id}/${glassObj.main_image}`;
...
$("#productForm").submit(function(e)
{
    ...
    if (product_name ==='') {
        document.getElementById("product_nameError" + class_name).textContent = "Product Detail required";
...
let dataListView = $('.datatables-basic')
    ...
    .on('draw.dt', function() 
    {
      $('#processingLoader').hide();
        ...
        "bFilter": false,
        "ajax": {
            url: "{{ route('admin.glassproduct.search') }}",
            "dataType": "json",
            "type": "POST",
            "data": function(d) 
            {
    ...
    $("table").delegate(".action-delete", "click", function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ url('/glassproduct') }}" + '/' + id + '/destroy',
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
