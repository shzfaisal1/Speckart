...
@endsection
@section('content')
@php $tbl_setting =  DB::table("tbl_product_code_setting")->where('product_type','Goggles')->first();   @endphp
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Add New Goggles</h5>
                        ...
                        <div class="multisteps-form__content1">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control"  placeholder="Enter Product Name" id="product_name" name="product_name" autocomplete="off">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter Color" name="Color" id="Color" autocomplete="off">
                                ...
                                @endif
                                @if($tbl_setting->size == '0')
                                <div class="col-md-4">
                                    <label for="">Size </label>
                                    <input type="text" class="form-control" placeholder="Enter Size" name="Size" id="Size" autocomplete="off">
                                    <div id="sizeListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                    <span class="error badge text-danger" id="SizeError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->type == '0')
                                <div class="col-md-4">
                                    <label for="">Type </label>
                                    <input type="text" class="form-control" placeholder="Enter Type" name="Type" id="Type" autocomplete="off">
                                ...
                                @endif
                                @if($tbl_setting->Shape == '0')
                                <div class="col-md-4">
                                    <label for="">Shape </label>
                                    <input type="text" class="form-control" placeholder="Enter Shape" name="Shape" id="Shape" autocomplete="off">
                                    <div id="shapeListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                    <span class="error badge text-danger" id="ShapeError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->Material == '0')
                                <div class="col-md-4">
                                    <label for="">Material </label>
                                    <input type="text" class="form-control" placeholder="Enter Material" name="Material" id="Material" autocomplete="off">
                                    <div id="materialListName" class="dropdown-menu" style="display: none; position: static;"></div>
                                    <span class="error badge text-danger" id="MaterialError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->Temple_Detail == '0')
                                <div class="col-md-4">
                                    <label for="">Temple Detail </label>
                                    <input type="text" class="form-control" placeholder="Enter Temple Detail" name="Temple_Detail" id="Temple_Detail">
                                    <span class="error badge text-danger" id="Temple_DetailError"></span>
                                </div>
                                @endif
                                @if($tbl_setting->Bridge_Size == '0')
                                <div class="col-md-4">
                                    <label for="">Bridge Size </label>
                                    <input type="text" class="form-control" placeholder="Enter Bridge Size" name="Bridge_Size" id="Bridge_Size">
                                    <span class="error badge text-danger" id="Bridge_SizeError"></span>
                                </div>
                                @endif
...
{
    document.getElementById('modalTitle').innerText = 'Add New Goggles';
    document.getElementById('productForm').action = '{{ route('admin.gogglesproduct.store') }}';
    document.getElementById('formMethod').value = 'POST';
    document.getElementById('productForm').reset();
    $('#productModal').modal('show');
}

function openEditModal(goggles) {
    var goggles = JSON.parse(decodeURIComponent(goggles));
    document.getElementById('modalTitle').innerText = 'Edit Goggles Product';
    document.getElementById('productForm').action = `{{ url('/gogglesproduct/` + goggles.id + `') }}`;
    document.getElementById('formMethod').value = 'PUT';
    document.getElementById('Purchase_Base_Price').value = goggles.Purchase_Base_Price;
    document.getElementById('Purchase_Price').value = goggles.Purchase_Price;
    document.getElementById('Retail_Price').value = goggles.Retail_Price;
    document.getElementById('BB_Price').value = goggles.BB_Price;
    document.querySelector(`input[name="Track_Inventory"][value="${goggles.Track_Inventory}"]`).checked = true;
    document.querySelector(`input[name="Allow_Negative_Inventory"][value="${goggles.Allow_Negative_Inventory}"]`).checked = true;
    document.getElementById('uid').value = goggles.id;
    document.getElementById('product_id').value = goggles.product_id;
    if (document.getElementById('product_code')) document.getElementById('product_code').value = goggles.product_code || '';
    if (document.getElementById('product_name')) document.getElementById('product_name').value = goggles.product_name || '';
    if (document.getElementById('Company')) document.getElementById('Company').value = goggles.Company || '';
    if (document.getElementById('Quality')) document.getElementById('Quality').value = goggles.Quality || '';
    if (document.getElementById('Color')) document.getElementById('Color').value = goggles.Color || '';
    if (document.getElementById('Size')) document.getElementById('Size').value = goggles.Size || '';
    if (document.getElementById('Type')) document.getElementById('Type').value = goggles.Type || '';
    if (document.getElementById('Gender')) document.getElementById('Gender').value = goggles.Gender || '';
    if (document.getElementById('Shape')) document.getElementById('Shape').value = goggles.Shape || '';
    if (document.getElementById('Material')) document.getElementById('Material').value = goggles.Material || '';
    if (document.getElementById('Temple_Detail')) document.getElementById('Temple_Detail').value = goggles.Temple_Detail || '';
    if (document.getElementById('Bridge_Size')) document.getElementById('Bridge_Size').value = goggles.Bridge_Size || '';
    if (document.getElementById('Description')) document.getElementById('Description').value = goggles.Description || '';
    
    try {
        var images = JSON.parse(goggles.product_image);
        ...
        if (Array.isArray(images) && images.length > 0) {
            images.forEach(function(imageName) {
                var imagePath = `/public/uploads/goggles/product/${goggles.product_id}/${imageName}`;
    ...
    if (goggles.main_image) {
        var mainImagePath = `/public/uploads/frame/goggles/${goggles.product_id}/${goggles.main_image}`;
...
let dataListView = $('.datatables-basic')
    ...
    .on('draw.dt', function() 
    {
      $('#processingLoader').hide();
        ...
        "bFilter": false,
        "ajax": {
            url: "{{ route('admin.gogglesproduct.search') }}",
            "dataType": "json",
            "type": "POST",
            "data": function(d) 
            {
    ...
    $("table").delegate(".action-delete", "click", function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ url('/gogglesproduct') }}" + '/' + id + '/destroy',
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
